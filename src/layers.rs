...
impl Layer {
    pub fn new(input_size: usize, output_size: usize, activation: Activation, rate: FloatPrecision) -> Self {
        ...
        let bias_data:Vec<FloatPrecision> = (0..output_size).map(|_| rng.gen_range(-1.0..1.0)).collect();
    ...
    pub fn backward(&mut self, input: &DMatrix, error: &DMatrix) {
        mwrap(self.activation.fd, &self.net, &mut self.fdnet); // f'(net)
        naive_mulm(error, &self.fdnet, &mut self.delta); // dE * f'(net)
        smulmt(self.rate, &self.delta, input, &mut self.dw); // dW
        smul(self.rate, &self.delta, &mut self.db); // db
